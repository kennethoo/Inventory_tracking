...
function addItem(){
    ...
    axios.post("http://localhost:4000/newinventoty",inventory).then((result)=>{
        getInventory()
        openInventory()
    })
    ...
    return (
      ...
      <p> Add Inventory</p>
      <button  onClick={()=>{openInventory()}}   className="close-that"><IoCloseSharp/></button>
        </div>
     ...
     <input  placeholder="Clothing" onChange={addCategorie}  value={categoty} className="fullname-profile" type="text" name="category" />
               ...
               <SelectWarehouse getInventory={getInventory} setWarehouse={setWarehouse} warehouse={warehouse}/>
                <div className="edit-box-profile">
				<p>Ventor</p>
        ...
        <input  placeholder="100" onChange={addPrice}  value={price} className="fullname-profile" type="number" name="price" />
                ...
                <button onClick={addItem} className="add-program">Add Item</button>
        </div>
